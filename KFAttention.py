...
class KFAttention(nn.Module):
    def __init__(
        ...
        self.W_Q = nn.Linear(state_dim, hopfield_dim, bias=False)                   # (d_r, d_h)
        self.W_K = nn.Linear(memory_dim, hopfield_dim, bias=False)                  # (d_y, d_k)
    ...
    def forward(self, queries, keys, values):
        Q = self._expmap0(self.W_Q(queries))                                        # (S, d_k + 1)
        K = self._expmap0(self.W_K(keys))                                           # (M, d_k + 1)
